...
class Regressor(nn.Module):
	...
	###train only:def forward(self, imgs, gt_heatmap, gt_landmark):
	def forward(self, imgs, gt_heatmap, gt_landmark):
		...
		for i in torch.arange(self.nstack):
			...
			if i < self.nstack - 1:
				#print("merge_features = " + str((self.merge_features[i](feature)).shape))
				#print("merge_preds = " + str((self.merge_preds[i](feature)).shape))
...
class GazeEstimator(nn.Module):
	...
	###trainonly:def forward(self, imgs, landmarks, gt_gaze):
	def forward(self, imgs, landmarks, gt_gaze):
		...
		#print('gaze:', gaze, 'gt_gaze:', gt_gaze)
		gaze_loss = self.gaze_loss(gaze, gt_gaze)
		#gaze_loss = torch.tensor(gaze_loss, dtype=torch.float32)
