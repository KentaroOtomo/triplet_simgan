...
def preprocess_unityeyes_image(img, json_data):
    ...
    def process_coords(coords_list):
        ...
        return np.array([(x, 96-y, z) for (x, y, z) in coords])
...
def get_heatmaps(w, h, landmarks):
    heatmaps = np.empty((0, 96, 160))
    i = 0
    while i <= 33:
    #for (y, x) in landmarks[i]:
        old_heatmaps = []
        peak = 0
        old_heatmaps.append(gaussian_2d(w, h, cx=landmarks[i][1], cy=landmarks[i][0], sigma=4.0))
        peak = np.amax(old_heatmaps)
        multiple = 255 / peak
        hm = np.multiply(old_heatmaps, multiple)
        #print()
        #print(hm.shape)
        heatmaps = np.vstack((heatmaps, hm))
        i = i + 1
